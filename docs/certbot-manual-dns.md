...
TXT no painel de DNS. O domínio `primati.com.br` está no registro.br, que
não tem API pública de DNS para automatizar isso — o processo é manual, leva
~2 minutos a cada emissão/renovação.
...
_acme-challenge.supabase.primati.com.br.
...
1. Entre em [registro.br](https://registro.br) → **Meus domínios** →
   `primati.com.br` → **DNS** → **Editar zona** (ou "Adicionar registro",
   nome exato do botão pode variar conforme a versão do painel).
2. Tipo: **TXT** · Nome/Host: `_acme-challenge.supabase` (o registro.br já
   adiciona o domínio base sozinho — se pedir o nome completo, use
   `_acme-challenge.supabase.primati.com.br`) · Valor: o texto exato que o
   certbot mostrou.
3. Espere a propagação (geralmente poucos minutos, pode levar mais no
   registro.br). Confirme com:
   ```bash
   dig +short TXT _acme-challenge.supabase.primati.com.br
   ```
   até aparecer o valor esperado — só então volte ao terminal do certbot e
   aperte Enter para continuar.
4. Sucesso grava os arquivos em
   `/etc/letsencrypt/live/supabase.primati.com.br/{fullchain,privkey}.pem`
...
Repita os passos do TXT no registro.br, aguarde propagar, confirme com
`dig`, deixe o certbot terminar, e reinicie o container do Nginx (`sh run.sh
restart nginx`).
...
Se `curl -vI https://supabase.primati.com.br:9443 2>&1 | grep expire`
