...
server {
    ...
    location / {
        ...
        sub_filter_once on;
        sub_filter '</body>' '<style>#__logout_fab{position:fixed;bottom:20px;right:20px;z-index:999999;width:32px;height:32px;border-radius:50%;background:#3ecf8e;color:#05261a;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,.3);text-decoration:none;opacity:.7;transition:opacity .15s,transform .15s}#__logout_fab:hover{opacity:1;background:#34b87c;transform:scale(1.08)}</style><a id="__logout_fab" href="/logout" title="Sair"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg></a><script>(function(){function ensureFnBtn(){if(window.location.pathname.indexOf("/functions")===-1)return;var els=document.querySelectorAll("a,button");var examplesBtn=null;for(var i=0;i<els.length;i++){if(els[i].textContent.trim()==="Examples"){examplesBtn=els[i];break;}}if(!examplesBtn)return;if(document.getElementById("__new_fn_btn"))return;var container=examplesBtn.parentElement;if(!container)return;var btn=document.createElement("button");btn.id="__new_fn_btn";btn.type="button";btn.textContent="Nova função";btn.style.cssText="margin-left:8px;padding:6px 14px;border:none;border-radius:6px;background:#3ecf8e;color:#05261a;font-weight:600;font-size:13px;cursor:pointer";btn.addEventListener("click",function(){var name=window.prompt("Nome da nova função (letras minúsculas, números, - ou _, começando com letra):");if(!name)return;name=name.trim();if(!/^[a-z][a-z0-9_-]{0,62}$/.test(name)||name==="main"){window.alert("Nome inválido.");return;}var code="Deno.serve(() => Response.json({ message: \"Hello from Edge Functions!\" }));\n";fetch("/admin/api/functions/"+encodeURIComponent(name),{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({code:code})}).then(function(r){return r.json().then(function(d){return{ok:r.ok,d:d};});}).then(function(res){if(!res.ok){window.alert(res.d&&res.d.error?res.d.error:"Não foi possível criar a função.");return;}window.location.reload();}).catch(function(){window.alert("Erro de rede ao criar a função.");});});container.appendChild(btn);}setInterval(ensureFnBtn,1000);ensureFnBtn();})();</script></body>';
    }
